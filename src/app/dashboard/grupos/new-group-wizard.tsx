...
import type { GroupWithMembers, ReservationSlim } from "./groups-view";
...
type NewGroupWizardProps = {
  rooms: Room[];
  existingReservations: ReservationSlim[];
  onCreated: (group: GroupWithMembers) => void;
};
...
export function NewGroupWizard({
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      onCreated(data as GroupWithMembers);
      setWizardOpen(false);
    } catch (err) {
      setError(
  ...
  return (
    <Dialog open={wizardOpen} onOpenChange={handleWizardOpenChange}>
      <DialogTrigger render={<Button size="sm" />}>Nuevo grupo</DialogTrigger>
                ...
                roomsByFloor.map(([floor, floorRooms]) => (
                  <div key={floor} className="flex flex-col gap-2">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium">Piso {floor}</span>
                      <Button
                        type="button"
                        variant="ghost"
                        size="sm"
          ...
          <div className="flex max-h-96 flex-col gap-3 overflow-y-auto">
            <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border bg-muted/40 px-3 py-2 text-sm">
              <span className="font-medium">
                {assignedCount} de {cantidadPersonasNum} personas asignadas —
                faltan {missingCount}
              </span>
              <Button
                type="button"
            ...
            {capacityShortfall > 0 ? (
              ...
              >
                Capacidad insuficiente: faltan {capacityShortfall} habitaciones
              </p>
            ) : null}
              ...
              >
                ...
                <div className="flex w-24 flex-col gap-1">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      updateMember(index, {
                        documentType: (value as DocumentType | null) ?? "DNI",
                      })
                    }
                  >
